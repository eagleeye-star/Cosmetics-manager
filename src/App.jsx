...
export default function App() {
  ...
  const [setupDone, setSetupDone] = useState(()=>!!localStorage.getItem("cosm_setup"));
  const [institution, setInstitution] = useState(()=>loadInstitution("cosmeticsManager_v1_license"));
  ...
  if (!setupDone) return <SetupWizard
    instLabel="Store"
    accentColor="#d946a8"
    bgGrad="linear-gradient(135deg,#0d0a0e,#1a0a1a)"
    onComplete={({instName,instAddr,username,pin})=>{
      setStaff([{id:"st1",name:username,role:"Owner",commissionRate:0,pin}]);
      setInstitution({name:instName,address:instAddr}); saveInstitution("cosmeticsManager_v1_license",{name:instName,address:instAddr});
      localStorage.setItem("cosm_setup","1"); setSetupDone(true);
    }}
  />;
  if (isExpired(license.expiry)) return <LicenseExpiredScreen license={license} onRenew={() => setLicense(null)} />;
